...
const toInt = i => {
  ...
  if (isNaN(remainder) || isNaN(forcedInt)) {
    return null;
  }
  return forcedInt;
};
...
const msToStamp = ms => {
  ...
  // consume working ms
  while (working >= sInHour) {
    working -= sInHour;
    hours++;
  }
  while (working >= sInMinute) {
    working -= sInMinute;
    minutes++;
  }
  ...
  return (
    result +
    (hours > 0 ? `${minutes.toFixed(0)}`.padStart(2, "0") : `${minutes}`) +
    ":" +
...
export { toInt, msToStamp, toDuration, arrayBufferToBase64 };
