...
describe("Adp.reps_", () => {
  ...
  it("should throw an error if no representations exist", () => {
    ...
    const promises = [
      new MPD({ data: inputA })
        .setup()
        .catch(e => expect(e).toEqual(outputA))
    ];
...
describe("Adp.bestRep", () => {
  ...
  it("should be capable of identifying a speed-based best rep", () => {
    ...
    const promises = [
      new MPD({ data: inputA }).setup().then(mpd => {
        expect(mpd.adps.map(a => a.bestRep(1000000).id)).toEqual(outputA);
      }),
      new MPD({ data: inputB }).setup().then(mpd => {
        expect(mpd.adps.map(a => a.bestRep(1000000).id)).toEqual(outputB);
      }),
      new MPD({ data: inputC }).setup().then(mpd => {
        expect(mpd.adps.map(a => a.bestRep(1000000).id)).toEqual(outputC);
      }),
      new MPD({ data: inputD }).setup().then(mpd => {
        expect(mpd.adps.map(a => a.bestRep(1000000).id)).toEqual(outputD);
      })
    ];
...
describe("Adp.matchBandwidth", () => {
  it("should be capable of identifying a nearest-representation", () => {
    ...
    const promises = [
      new MPD({ data: inputA }).setup().then(mpd => {
        expect(mpd.adps.map(a => a.matchBandwidth(1000000).id)).toEqual(
          outputA
        );
      }),
      new MPD({ data: inputB }).setup().then(mpd => {
